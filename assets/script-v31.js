...
(function () {
  ...
  // + per-scroll canvas decode). Play a lightweight looping hero video instead.
  ...
  if (coarse) {
    ...
    if (vid && !reduced) {
      ...
      function startHeroVideo() {
        ...
        } catch (e) { /* autoplay blocked; poster remains visible */ }
      }
      window.addEventListener('scroll', startHeroVideo, { passive: true });
      window.addEventListener('touchstart', startHeroVideo, { passive: true });
      window.addEventListener('load', function () { setTimeout(startHeroVideo, 500); });
    }
    return;
  }
